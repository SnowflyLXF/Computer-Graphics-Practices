...
string textureName = "/Users/lixuefei/Desktop/starter/goldy.ppm";
...
unsigned char* brighten(unsigned char* img_data, int length);
unsigned char* darken(unsigned char* img_data, int length);
...
unsigned char* loadImage(int& img_w, int& img_h){
   ...
   ppmFile >> img_w >> img_h;
   unsigned char* img_data = new unsigned char[4*img_w*img_h];  //RGB and transparency
...
while(ppmFile>>red>>green>>blue){
   printf("pixel number: %5d, R: %d, G: %d, B: %d\n", pixelNum, red, green, blue);
   ...
   for (int i = img_h-1; i >= 0; i--){
      ...
      for (int j = img_w-1; j >=0 ; j--){
         ...
         img_data[i*img_w*4 + j*4] = red_data[p];  //Red
         img_data[i*img_w*4 + j*4 + 1] = green_data[p];  //Green
         img_data[i*img_w*4 + j*4 + 2] = blue_data[p];  //Blue
...
void updateVertices(){ 
   float vx = -g_size*sin(g_angle) + g_size*cos(g_angle);
   float vy =  g_size*sin(g_angle) + g_size*cos(g_angle);
   vertices[0] = g_pos_x + vx;  //Top right x
   vertices[1] = g_pos_y + vy;  //Top right y
   
   vx = g_size*sin(g_angle) + g_size*cos(g_angle);
   vy = -g_size*cos(g_angle) + g_size*sin(g_angle);
   ...
   vx = g_size*sin(g_angle) - g_size*cos(g_angle);
   vy = -g_size*sin(g_angle) - g_size*cos(g_angle);
...
void mouseClicked(float m_x, float m_y){   
   ...
   float x = m_x*cos(g_angle) + m_y*sin(g_angle) - g_pos_x;  
   float y = m_y*cos(g_angle) - m_x*sin(g_angle)- g_pos_y;
   x = x / g_size;
   y = y / g_size;
   ...
   if (x < .9 && x > -.9 && y < .9 && y > -.9){ //TODO: Test your understanding: What happens if you change .9 to .8?
      g_bTranslate = true;
        ...
        if ((x <= 1.05 && x >= .9 && y<= 1.05 && y >= .9) || (x >= -1.05 && x <= -.9 && y <= 1.05 && y >= .9) || (x <= 1.05 && x >= .9 && y >= -1.05 && y <= -.9) || (x >= -1.05 && x <= -.9 && y >= -1.05 && y <= -.9)){
          g_bRotate = true;
        }
        else
          g_bScale = true;
...
void mouseDragged(float m_x, float m_y){   
   ...
   if (g_bScale){
      ...
      if (g_clicked_x <= g_lastCenter_x+g_clicked_size+.05 && g_clicked_x >= g_lastCenter_x+g_clicked_size-.1)
        g_size = g_clicked_size + m_x - g_clicked_x;
      if (g_clicked_x <= g_lastCenter_x-g_clicked_size+.1 && g_clicked_x >= g_lastCenter_x-g_clicked_size-.05)
        g_size = g_clicked_size - m_x + g_clicked_x;
      if (g_clicked_y <= g_lastCenter_y+g_clicked_size+.05 && g_clicked_y >= g_lastCenter_y+g_clicked_size-.1)
        g_size = g_clicked_size + m_y - g_clicked_y;
      if (g_clicked_y <= g_lastCenter_y-g_clicked_size+.1 && g_clicked_y >= g_lastCenter_y-g_clicked_size-.05)
        g_size = g_clicked_size - m_y + g_clicked_y;
   }
   
   if (g_bRotate){
      // Compute the new angle, g_angle, based on the mouse positions
      g_angle = g_clicked_angle - atan(((m_x- g_lastCenter_x) * (g_clicked_y-g_lastCenter_y) - (g_clicked_x-g_lastCenter_x) * (m_y-g_lastCenter_y)) / ((g_clicked_x-g_lastCenter_x) * (m_x-g_lastCenter_x) + (g_clicked_y-g_lastCenter_y) * (m_y-g_lastCenter_y)));
   }
...
GLchar *swirlSource =
    ...
    "   vec2 st = texcoord;"
    "   float Radius = Res * uR * sin(u_time/10);"
    "   vec2 xy = Res * st;"
    "   vec2 dxy = xy - vec2(Res/2., Res/2.);"
    "   float r = length(dxy);"
    "   float beta = atan(dxy.y, dxy.x) + radians(uD) * 2.0 * (-(r/Radius)*(r/Radius) + 1.0) + u_time;"
...
int main(int argc, char *argv[]){
  ...
  glCompileShader(fragmentShader);
   ...
   GLuint swirlShader = glCreateShader(GL_FRAGMENT_SHADER);
   glShaderSource(swirlShader, 1, &swirlSource, NULL);
   glCompileShader(swirlShader);
  ...
  glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &status);
   ...
   glGetShaderiv(swirlShader, GL_COMPILE_STATUS, &status);
   if (!status){
      char buffer[512];
      glGetShaderInfoLog(swirlShader, 512, NULL, buffer);
      printf("Swirl Shader Compile Failed. Info:\n\n%s\n",buffer);
   }
   ...
   glAttachShader(shaderProgram2, swirlShader);
      ...
      while (SDL_PollEvent(&windowEvent)){  //Process input events (e.g., mouse & keyboard)
        ...
        if(windowEvent.type == SDL_KEYUP)
            if (windowEvent.key.keysym.sym == SDLK_UP){
              img_data = brighten(img_data, 4*img_w*img_h);
              glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img_w, img_h, 0, GL_RGBA, GL_UNSIGNED_BYTE, img_data);
              glGenerateMipmap(GL_TEXTURE_2D);
            }
            if (windowEvent.key.keysym.sym == SDLK_DOWN){
              img_data = darken(img_data, 4*img_w*img_h);
              glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img_w, img_h, 0, GL_RGBA, GL_UNSIGNED_BYTE, img_data);
              glGenerateMipmap(GL_TEXTURE_2D);
            }
            if (windowEvent.key.keysym.sym == SDLK_a){
              ...
              cl += 0.2;
              int timeUniformLocation = glGetUniformLocation(shaderProgram, "u_time");
              glUniform1f(timeUniformLocation, cl);
